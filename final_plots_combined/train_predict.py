...
fluctuation = np.mean(np.abs(y_pred_flat - mean_pred))

print(f"\nRBF ELM MAE: {mae:.2f}, RMSE: {rmse:.2f}")
print(f"Fluctuation Across the Mean (FAT): {fluctuation:.2f}")
...
def get_rbf_predictions():
    return dates, y_true.flatten(), y_pred.flatten()
